...
func Test_getModuleVersion(t *testing.T) {
	tests := []struct {
		...
		{"GIVEN a string with an incorrect version y.z format THEN return any empty string", "foobar=1.2", ""},
...
func TestGetModuleNameAndVersion(t *testing.T) {
	tests := []struct {
		...
		{"GIVEN a string with a repo name and an invalid version THEN return the repo name, bool if there's a version and an empty string", "github.com/ministryofjustice/foo?ref=1.2", "foo", true, ""},
